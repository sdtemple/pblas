#' Standard PBLA (Parallel)
#'
#' Compute pair-based likelihood approximation. Supports Erlang infectious periods. For parallel computing, `parallel::makeCluster`, `doParallel::registerDoParallel`, and `parallel::stopCluster`.
#'
...
#'
#' @export
pbla_std_parallel = function(r, beta, gamma, m = 1, A = 1, lag = 0){
  ...
  if((any(beta <= 0)) | (any(gamma <= 0)) |
    ...
    if(m == 1){ # exponential infectious periods
      ...
      psichi = foreach::foreach(j = 1:n, .combine = c) %dopar% {
        X = 0
        Y = 0
        rj= r[j]
        deltaj = delta[j]
